...
async def start(message: types.Message):
    if not db.user_exists(message.from_user.id):
        ...
        db.set_balance(5000, message.from_user.id)
...
@dp.callback_query_handler(lambda x: x.data == "convert")
async def converter(callback_query: types.CallbackQuery):
    await bot.send_message(callback_query.from_user.id,
                           "Отправьте отношение валют и колличество валюты\nПример - 100 USD/EUR")
    await ConverterStates.waiting_for_valute.set()


@dp.message_handler(state=ConverterStates.waiting_for_valute)
async def convert_valute(message: types.Message, state: FSMContext):
    ...
    try:
        amount = int(mes.split()[0])
        valutes = mes.split()[1].split("/")
        valut = data_curse['Valute'][valutes[0].upper()]['Value'] / data_curse['Valute'][valutes[1].upper()]['Value']
        await bot.send_chat_action(message.from_user.id, ChatActions.TYPING)
        await asyncio.sleep(2)
        await bot.send_message(message.from_user.id, text="{:.2f} {}".format(valut * amount, valutes[1]))
        await state.finish()
    except ValueError:
        await bot.send_message(message.from_user.id,
                               "Ошибка! Отправьте отношение валют и колличество валюты корректно\nПример - 100 USD/EUR")
    except KeyError:
        await bot.send_message(message.from_user.id,
                               "Ошибка! Отправьте отношение валют и колличество валюты корректно\nПример - 100 USD/EUR")
...
async def curse(callback_query: types.CallbackQuery):
    ...
    if valut == "usd":
        ...
        if float(data_curse['Valute'][valut.upper()]['Value']) > float(
                data_curse['Valute'][valut.upper()]['Previous']):
            await bot.send_message(callback_query.from_user.id,
                                   "Доллар к рублю:\n" + str(
...
async def start_bot(callback_query: types.CallbackQuery):
    ...
    У вас {db.get_balance(user_id)} очков 💵. Вы можете ставить ставки и играть в игры🎮.
...
async def cmd_balance(message: types.Message):
    ...
    await message.answer(f'Ваш баланс: {db.get_balance(user_id)}\n')
...
async def cmd_balance(message: types.Message):
    ...
    if pay_balance == config.BALANCE_PAY:
        if int(db.get_balance(user_id)) <= 0:
            db.set_balance(100, user_id)
            await message.answer(f'Вы успешно пополнили баланс, ваш баланс составляет 100💸')
        else:
            await message.answer(f'Ваш баланс: {db.get_balance(user_id)} 💸, вы не можете пополнить баланс.')
...
async def get_user_bet(message: types.Message, state: FSMContext):
    ...
    user_id = message.from_user.id
    if int(bet) > int(db.get_balance(user_id)):
...
async def get_user_bet(message: types.Message, state: FSMContext):
    ...
    if bot_data > user_data:
        db.set_balance(int(db.get_balance(user_id)) - bet, user_id)
        await bot.send_message(message.from_user.id, f"Вы проиграли! Ваш баланс: {db.get_balance(user_id)}")
    elif bot_data < user_data:
        db.set_balance(int(db.get_balance(user_id)) + bet, user_id)
        await bot.send_message(message.from_user.id, f"Вы выиграли! Ваш баланс: {db.get_balance(user_id)}")
    else:
        await bot.send_message(message.from_user.id, f"Ничья! Ваш баланс: {db.get_balance(user_id)}")
...
async def get_user_bet(message: types.Message, state: FSMContext):
    ...
    point = text[-1]
    db.set_balance(int(db.get_balance(user_id)) + point, user_id)
...
async def get_user_bet(message: types.Message, state: FSMContext):
    ...
    if choice_rps == q:
        ...
        db.get_balance(int(db.get_balance(user_id) + bet, user_id))
        await bot.send_message(message.from_user.id, f"Вы выиграли! Ваш баланс: {db.get_balance(user_id)}")
...
async def end_que(callback_query: types.CallbackQuery):
    current_ball = db.get_quiz(callback_query.from_user.id)
    max_ball = get_max_ball(db.get_quiz_lvl(callback_query.from_user.id))
    if current_ball * 100 // max_ball >= 66:
        await bot.send_message(callback_query.from_user.id, "Поздравляем! У вас отличный результат")
    elif 40 <= current_ball * 100 // max_ball < 66:
        await bot.send_message(callback_query.from_user.id, "Неплохой результат, но еще есть к чему стремиться :)")
    else:
        await bot.send_message(callback_query.from_user.id,
                               "К сожалению у вас плохой результат, советую узнать побольше информации о финансовой грамотности\nЭто можно сделать через Меню ➡ Справочная информация")
    await bot.send_message(callback_query.from_user.id, f"Вы набрали - {current_ball} из {max_ball} баллов")
